...
namespace pcpp
{
	...
	SystemCore PfRingDevice::getCurrentCoreId() const { return SystemCores::IdToSystemCore[sched_getcpu()]; }
		...
		for (int i = 0; i < m_NumOfOpenedRxChannels; i++)
		{
			...
			if (res < 0)
			{
				if (res == PF_RING_ERROR_NOT_SUPPORTED)
					PCPP_LOG_ERROR("BPF filtering isn't supported on current PF_RING version. Please re-compile "
								   "PF_RING with the --enable-bpf flag");
				else
	...
	bool PfRingDevice::isFilterCurrentlySet() const { return m_IsFilterCurrentlySet; }
...
#define MAX_TRIES 5
		...
		while (tries < MAX_TRIES)
		{
			...
			{
				tries++;
				PCPP_LOG_DEBUG("Try #" << tries
									   << ": Got ENOBUFS (write buffer full) error while sending packet. Sleeping 20 "
										  "usec and trying again");
				usleep(2000);
			}
			else
				break;
