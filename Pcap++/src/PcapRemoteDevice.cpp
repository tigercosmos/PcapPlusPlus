...
namespace pcpp
{
	...
	bool PcapRemoteDevice::open()
	{
		...
		// catches all traffic
		if (!setFilter("ether proto (\\ip or \\ip6 or \\arp or \\rarp or \\decnet or \\sca or \\lat or \\mopdl or "
					   "\\moprc or \\iso or \\stp or \\ipx or \\netbeui or 0x80F3)")) // 0x80F3 == AARP
		{
	...
	ThreadStart PcapRemoteDevice::getCaptureThreadStart() { return &remoteDeviceCaptureThreadMain; }
