...
namespace pcpp
{
	...
	void BPFStringFilter::parseToString(std::string &result) { result = m_FilterStr; }

	bool BPFStringFilter::verifyFilter() { return m_BpfWrapper.setFilter(m_FilterStr); }
	...
	PortFilter::PortFilter(uint16_t port, Direction dir) : IFilterWithDirection(dir) { portToString(port); }
	...
	void CompositeFilter::setFilters(const std::vector<GeneralFilter *> &filters) { m_FilterList = filters; }
