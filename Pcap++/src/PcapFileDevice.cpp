...
namespace pcpp
{
	...
	static bool checkNanoSupport()
	{
		...
		PCPP_LOG_DEBUG(
			...
			"recompile PcapPlusPlus with nano precision support to use this feature. Using default microsecond "
			"precision");
		return false;
#endif
	}
	...
	IFileDevice::IFileDevice(const std::string &fileName) : IPcapDevice() { m_FileName = fileName; }
	...
	bool PcapFileReaderDevice::isNanoSecondPrecisionSupported() { return checkNanoSupport(); }
	...
	bool PcapFileReaderDevice::getNextPacket(RawPacket &rawPacket)
	{
		...
		timespec ts = {
			pkthdr.ts.tv_sec,
			static_cast<long>(
				pkthdr.ts.tv_usec)}; // because we opened with nano second precision 'tv_usec' is actually nanos
#else
	...
	bool PcapFileWriterDevice::isNanoSecondPrecisionSupported() { return checkNanoSupport(); }
	...
	bool PcapFileWriterDevice::open()
	{
		...
		case LINKTYPE_DLT_RAW2:
			PCPP_LOG_ERROR("The only Raw IP link type supported in libpcap/WinPcap/Npcap is LINKTYPE_DLT_RAW1, please "
						   "use that instead");
			return false;
		default:
			break;
	...
	bool PcapNgFileWriterDevice::writePacket(RawPacket const &packet) { return writePacket(packet, std::string()); }
