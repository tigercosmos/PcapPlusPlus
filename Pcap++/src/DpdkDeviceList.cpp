...
namespace pcpp
{
	...
	bool DpdkDeviceList::initDpdkDevices(uint32_t mBufPoolSizePerDevice, uint16_t mBufDataSize)
	{
		if (!m_IsDpdkInitialized)
		{
			PCPP_LOG_ERROR("DPDK is not initialized!! Please call DpdkDeviceList::initDpdk(coreMask, "
						   "mBufPoolSizePerDevice) before start using DPDK devices");
			return false;
		}
	...
	SystemCore DpdkDeviceList::getDpdkMasterCore() const { return SystemCores::IdToSystemCore[GET_MASTER_CORE()]; }
	...
	bool DpdkDeviceList::writeDpdkLogToFile(FILE *logFile) { return (rte_openlog_stream(logFile) == 0); }
