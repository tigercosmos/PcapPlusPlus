...
#define DPDK_CONFIG_ETH_RSS_NVGRE RTE_ETH_RSS_NVGRE
	...
	bool DpdkDevice::configurePort(uint8_t numOfRxQueues, uint8_t numOfTxQueues)
	{
		...
		if (isVirtual())
		{
			...
			{
				PCPP_LOG_ERROR("Num of RX queues must be power of 2 when device is virtual (because of DPDK "
...
#define PACKET_TRANSMISSION_THRESHOLD 0.8
		...
		while (packetIndex < arrLength)
		{
			...
			else
			{
				...
				if (unlikely(mBufArrIndex == MAX_BURST_SIZE))
				{
					...
					{
						PCPP_LOG_DEBUG("Since NIC couldn't send all packet in this iteration, waiting for 0.2 second "
									   "for H/W descriptors to get free");
						usleep(200000);
	...
	int DpdkDevice::getAmountOfFreeMbufs() const { return (int)rte_mempool_avail_count(m_MBufMempool); }

	int DpdkDevice::getAmountOfMbufsInUse() const { return (int)rte_mempool_in_use_count(m_MBufMempool); }
