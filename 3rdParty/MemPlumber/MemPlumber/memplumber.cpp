...
class MemPlumberInternal
{
  private:
	...
	bool isVerbose() { return m_Verbose && m_Dumper != NULL; }
  ...
  public:
	...
	void programStarted() { m_ProgramStarted = 1; }
...
const char *getCaller() { return "Unknown"; }
#else
#include <execinfo.h>
...
void *operator new[](size_t size) _THROW_BAD_ALLOC { return operator new(size, getCaller(), 0); }

void *operator new(size_t size) _THROW_BAD_ALLOC { return operator new(size, getCaller(), 0); }

void *operator new(size_t size, const std::nothrow_t &) _NOEXCEPT { return operator new(size, getCaller(), 0); }
...
void operator delete(void *pointer) throw() { operator delete(pointer, __FILE__, __LINE__); }

void operator delete(void *pointer, std::size_t size) { operator delete(pointer, __FILE__, __LINE__); }

void operator delete[](void *pointer) _NOEXCEPT { operator delete(pointer, __FILE__, __LINE__); }

void operator delete[](void *pointer, std::size_t size) { operator delete(pointer, __FILE__, __LINE__); }

void operator delete[](void *pointer, const char *file, int line) { operator delete(pointer, file, line); }

void operator delete(void *pointer, const std::nothrow_t &) throw() { operator delete(pointer); }

void operator delete[](void *pointer, const std::nothrow_t &) throw() { operator delete(pointer, std::nothrow); }
...
void __stop() { MemPlumberInternal::getInstance().stop(); }
...
void __program_started() { MemPlumberInternal::getInstance().programStarted(); }
