...
namespace pcpp
{
	...
	uint16_t VlanLayer::getVlanID() const { return be16toh(getVlanHeader()->vlan) & 0xFFF; }

	uint8_t VlanLayer::getCFI() const { return ((be16toh(getVlanHeader()->vlan) >> 12) & 1); }

	uint8_t VlanLayer::getPriority() const { return (be16toh(getVlanHeader()->vlan) >> 13) & 7; }
