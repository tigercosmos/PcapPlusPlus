...
namespace pcpp
{
	...
	DnsLayer::DnsLayer(const DnsLayer &other) : Layer(other) { init(other.m_OffsetAdjustment, true); }
	...
	size_t DnsLayer::getBasicHeaderSize() { return sizeof(dnshdr) + m_OffsetAdjustment; }
	...
	size_t DnsLayer::getQueryCount() const { return be16toh(getDnsHeader()->numberOfQuestions); }
	...
	size_t DnsLayer::getAnswerCount() const { return be16toh(getDnsHeader()->numberOfAnswers); }
	...
	size_t DnsLayer::getAuthorityCount() const { return be16toh(getDnsHeader()->numberOfAuthority); }
	...
	DnsResource *DnsLayer::getFirstAdditionalRecord() const { return m_FirstAdditional; }
	...
	size_t DnsLayer::getAdditionalRecordCount() const { return be16toh(getDnsHeader()->numberOfAdditional); }
	...
	uint16_t DnsOverTcpLayer::getTcpMessageLength() { return be16toh(*(uint16_t *)m_Data); }

	void DnsOverTcpLayer::setTcpMessageLength(uint16_t value) { ((uint16_t *)m_Data)[0] = htobe16(value); }

	void DnsOverTcpLayer::computeCalculateFields() { setTcpMessageLength(m_DataLen - sizeof(uint16_t)); }
