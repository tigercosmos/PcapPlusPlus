...
namespace pcpp
{
	...
	class IPv4FragmentWrapper : public IPFragmentWrapper
	{
	  public:
		...
		uint16_t getFragmentOffset() override { return m_IPLayer->getFragmentOffset(); }

		uint32_t getFragmentId() override { return (uint32_t)be16toh(m_IPLayer->getIPv4Header()->ipId); }
		...
		size_t getIPLayerPayloadSize() override { return m_IPLayer->getLayerPayloadSize(); }
	...
	class IPv6FragmentWrapper : public IPFragmentWrapper
	{
	  public:
		...
		uint32_t getFragmentId() override { return be32toh(m_FragHeader->getFragHeader()->id); }
