...
namespace pcpp
{
	...
	void NtpLayer::setLeapIndicator(LeapIndicator val) { getNtpHeader()->leapIndicator = val; }
	...
	int8_t NtpLayer::getPollInterval() const { return getNtpHeader()->pollInterval; }

	void NtpLayer::setPollInterval(int8_t val) { getNtpHeader()->pollInterval = val; }

	double NtpLayer::getPollIntervalInSecs() const { return pow(2, getPollInterval()); }
	...
	double NtpLayer::getRootDelayInSecs() const { return convertFromShortFormat(getRootDelay()); }

	void NtpLayer::setRootDelayInSecs(double val) { getNtpHeader()->rootDelay = convertToShortFormat(val); }

	uint32_t NtpLayer::getRootDispersion() const { return getNtpHeader()->rootDispersion; }

	void NtpLayer::setRootDispersion(uint32_t val) { getNtpHeader()->rootDispersion = val; }

	double NtpLayer::getRootDispersionInSecs() const { return convertFromShortFormat(getRootDispersion()); }

	void NtpLayer::setRootDispersionInSecs(double val) { getNtpHeader()->rootDispersion = convertToShortFormat(val); }

	uint32_t NtpLayer::getReferenceIdentifier() const { return getNtpHeader()->referenceIdentifier; }

	void NtpLayer::setReferenceIdentifier(IPv4Address val) { getNtpHeader()->referenceIdentifier = val.toInt(); }
		...
		if (stratum == 0)
		{
			...
			case 4:
			{
				...
				case KissODeath::RATE:
					return "Rate exceeded.  The server has temporarily denied access because the client exceeded the "
						   "rate "
						   "threshold";
	...
	uint64_t NtpLayer::getReferenceTimestamp() const { return getNtpHeader()->referenceTimestamp; }

	void NtpLayer::setReferenceTimestamp(uint64_t val) { getNtpHeader()->referenceTimestamp = val; }

	double NtpLayer::getReferenceTimestampInSecs() const { return convertFromTimestampFormat(getReferenceTimestamp()); }
	...
	std::string NtpLayer::getReferenceTimestampAsString() { return convertToIsoFormat(getReferenceTimestamp()); }

	uint64_t NtpLayer::getOriginTimestamp() const { return getNtpHeader()->originTimestamp; }

	void NtpLayer::setOriginTimestamp(uint64_t val) { getNtpHeader()->originTimestamp = val; }

	double NtpLayer::getOriginTimestampInSecs() const { return convertFromTimestampFormat(getOriginTimestamp()); }
	...
	std::string NtpLayer::getOriginTimestampAsString() { return convertToIsoFormat(getOriginTimestamp()); }

	uint64_t NtpLayer::getReceiveTimestamp() const { return getNtpHeader()->receiveTimestamp; }

	void NtpLayer::setReceiveTimestamp(uint64_t val) { getNtpHeader()->receiveTimestamp = val; }

	double NtpLayer::getReceiveTimestampInSecs() const { return convertFromTimestampFormat(getReceiveTimestamp()); }
	...
	std::string NtpLayer::getReceiveTimestampAsString() { return convertToIsoFormat(getReceiveTimestamp()); }

	uint64_t NtpLayer::getTransmitTimestamp() const { return getNtpHeader()->transmitTimestamp; }

	void NtpLayer::setTransmitTimestamp(uint64_t val) { getNtpHeader()->transmitTimestamp = val; }

	double NtpLayer::getTransmitTimestampInSecs() const { return convertFromTimestampFormat(getTransmitTimestamp()); }
	...
	std::string NtpLayer::getTransmitTimestampAsString() { return convertToIsoFormat(getTransmitTimestamp()); }
	...
	bool NtpLayer::isDataValid(const uint8_t *data, size_t dataSize) { return data && dataSize >= sizeof(ntp_header); }
