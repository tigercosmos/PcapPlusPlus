...
namespace pcpp
{
	...
	bool Sll2Layer::isDataValid(const uint8_t *data, size_t dataLen) { return data && dataLen >= sizeof(sll2_header); }

	std::string Sll2Layer::toString() const { return "Linux cooked header v2"; }

	uint16_t Sll2Layer::getProtocolType() const { return be16toh(getSll2Header()->protocol_type); }

	void Sll2Layer::setProtocolType(uint16_t protocolType) { getSll2Header()->protocol_type = htobe16(protocolType); }

	uint32_t Sll2Layer::getInterfaceIndex() const { return be32toh(getSll2Header()->interface_index); }
	...
	uint16_t Sll2Layer::getArphrdType() const { return be16toh(getSll2Header()->ARPHRD_type); }

	void Sll2Layer::setArphrdType(uint16_t arphrdType) { getSll2Header()->ARPHRD_type = htobe16(arphrdType); }

	uint8_t Sll2Layer::getPacketType() const { return getSll2Header()->packet_type; }

	void Sll2Layer::setPacketType(uint8_t packetType) { getSll2Header()->packet_type = packetType; }

	uint8_t Sll2Layer::getLinkLayerAddrLen() const { return getSll2Header()->link_layer_addr_len; }

	const uint8_t *Sll2Layer::getLinkLayerAddr() const { return getSll2Header()->link_layer_addr; }
