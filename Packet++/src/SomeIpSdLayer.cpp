...
namespace pcpp
{
	...
	uint16_t SomeIpSdEntry::getServiceId() const { return be16toh(getSomeIpSdEntryHeader()->serviceID); }

	void SomeIpSdEntry::setServiceId(uint16_t serviceId) { getSomeIpSdEntryHeader()->serviceID = htobe16(serviceId); }

	uint16_t SomeIpSdEntry::getInstanceId() const { return be16toh(getSomeIpSdEntryHeader()->instanceID); }
	...
	uint32_t SomeIpSdEntry::getMinorVersion() const { return be32toh(getSomeIpSdEntryHeader()->data); }
