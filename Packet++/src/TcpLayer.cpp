...
#define TCPOPT_DUMMY 0xff
	...
	uint16_t TcpLayer::getSrcPort() const { return be16toh(getTcpHeader()->portSrc); }

	uint16_t TcpLayer::getDstPort() const { return be16toh(getTcpHeader()->portDst); }
	...
	TcpLayer::TcpLayer(const TcpLayer &other) : Layer(other) { copyLayerData(other); }
