...
#define IPV4_MAX_OPT_SIZE 40
	...
	IPv4OptionBuilder::IPv4OptionBuilder(const IPv4TimestampOptionValue &timestampValue)
	{
		...
		if (timestampValue.type == IPv4TimestampOptionValue::TimestampsForPrespecifiedIPs)
		{
			PCPP_LOG_ERROR("Cannot build timestamp option of type "
						   "IPv4TimestampOptionValue::TimestampsForPrespecifiedIPs - this type is not supported");
			m_BuilderParamsValid = false;
			return;
		}
		...
		{
			PCPP_LOG_ERROR("Cannot build timestamp option of type IPv4TimestampOptionValue::TimestampAndIP because "
	...
	IPv4Option IPv4OptionBuilder::build() const
	{
		...
		if ((recType == (uint8_t)IPV4OPT_NOP || recType == (uint8_t)IPV4OPT_EndOfOptionsList))
		{
			if (m_RecValueLen != 0)
			{
				PCPP_LOG_ERROR("Can't set IPv4 NOP option or IPv4 End-of-options option with size different than 0, "
							   "tried to set size "
							   << (int)m_RecValueLen);
				return IPv4Option(nullptr);
			}
	...
	IPv4Layer::IPv4Layer(const IPv4Layer &other) : Layer(other) { copyLayerData(other); }
	...
	bool IPv4Layer::isFirstFragment() const { return isFragment() && (getFragmentOffset() == 0); }
	...
	uint8_t IPv4Layer::getFragmentFlags() const { return getIPv4Header()->fragmentOffset & 0xE0; }
