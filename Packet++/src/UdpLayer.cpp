...
namespace pcpp
{
	...
	uint16_t UdpLayer::getSrcPort() const { return be16toh(getUdpHeader()->portSrc); }

	uint16_t UdpLayer::getDstPort() const { return be16toh(getUdpHeader()->portDst); }
		...
		if (m_PrevLayer != nullptr)
		{
			...
			if (m_PrevLayer->getProtocol() == IPv4)
			{
				...
				checksumRes = pcpp::computePseudoHdrChecksum(
					(uint8_t *)udpHdr, getDataLen(), IPAddress::IPv4AddressType, PACKETPP_IPPROTO_UDP, srcIP, dstIP);
