...
namespace pcpp
{
												 ...
												 bool cleanWholeFragList)
	{
		...
		do
		{
			PCPP_LOG_DEBUG("Starting first iteration of checkOutOfOrderFragments - looking for fragments that match "
						   "the current sequence or have smaller sequence");
			...
			do
			{
				...
				while (index < (int)tcpReassemblyData->twoSides[sideIndex].tcpFragmentList.size())
				{
					...
					if (SEQ_LT(curTcpFrag->sequence, tcpReassemblyData->twoSides[sideIndex].sequence))
					{
						...
						if (SEQ_GT(newSequence, tcpReassemblyData->twoSides[sideIndex].sequence))
						{
							...
							PCPP_LOG_DEBUG("Found a fragment in the out-of-order list which its sequence is lower than "
										   ...
										   << curTcpFrag->dataLength << " on side " << sideIndex
										   << ", new data size is " << (int)(curTcpFrag->dataLength - newLength));
						...
						{
							PCPP_LOG_DEBUG("Found a fragment in the out-of-order list which doesn't contain any new "
					...
					// if got to here it means the fragment has higher sequence than current sequence, increment index
					// and continue
					index++;
				}
