  ...
  protected:
	...
	uint32_t getSrcIPValue(pcpp::Packet &packet)
	{
		if (packet.isPacketOfType(pcpp::IPv4))
			...
			return pcpp::fnvHash((uint8_t *)packet.getLayerOfType<pcpp::IPv6Layer>()->getSrcIPv6Address().toBytes(),
								 16);
		else
			return 0;
	}
	...
	uint32_t getDstIPValue(pcpp::Packet &packet)
	{
		if (packet.isPacketOfType(pcpp::IPv4))
			...
			return pcpp::fnvHash((uint8_t *)packet.getLayerOfType<pcpp::IPv6Layer>()->getDstIPv6Address().toBytes(),
								 16);
		else
			return 0;
	}
